...
function printHelp () {
  echo "Usage: "
  echo "  cfdn.sh up|down|restart|generate|upgrade [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-i <imagetag>]"
  echo "  cfdn.sh -h|--help (print this message)"
  ...
  echo
  echo "	cfdn.sh generate -c mychannel"
  echo "	cfdn.sh up -c mychannel -s couchdb"
  echo "        cfdn.sh up -c mychannel -s couchdb -i 1.1.0-alpha"
  echo "	cfdn.sh up -l node"
  echo "	cfdn.sh down -c mychannel"
  echo "        cfdn.sh upgrade -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	cfdn.sh generate"
  echo "	cfdn.sh up"
  echo "	cfdn.sh down"
}
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS ; do
     ...
     if [ $? -eq 0 ] ; then
       echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of CFDN and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
       exit 1
     fi

     echo "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
     if [ $? -eq 0 ] ; then
       echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of CFDN and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
       exit 1
     fi
  done
}
...
function upgradeNetwork () {
  ...
  for PEER in peer0.comp1.example.com peer1.comp1.example.com peer0.comp2.example.com peer1.comp2.example.com; do
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/comp1.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/comp2.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``Comp1`` & ``Comp2``) each managing and maintaining two peer nodes.
...
# (``peer0.comp1.example.com`` & ``peer0.comp2.example.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Comp1MSP..."
    exit 1
  fi
  ...
  ./channel-artifacts/Comp2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Comp2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Comp2MSP..."
    exit 1
  fi
  echo
}
