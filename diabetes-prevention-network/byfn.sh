...
docker-compose $COMPOSE_FILES stop orderer.ehealthontario.ca
 # docker cp -a orderer.example.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.example.com
docker cp -a orderer.ehealthontario.ca:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.ehealthontario.ca
#  docker-compose $COMPOSE_FILES up -d --no-deps orderer.example.com
docker-compose $COMPOSE_FILES up -d --no-deps orderer.ehealthontario.ca

  for PEER in peer0.ehealthontario.ca peer1.ehealthontario.ca peer0.appletreemedicalgroup.com peer1.appletreemedicalgroup.com peer0.sunlife.ca peer1.sunlife.ca; do
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/ehealthontario.ca/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/appletreemedicalgroup.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/sunlife.ca/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/eHealthMSPanchors.tx -channelID $CHANNEL_NAME -asOrg eHealthMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for eHealthMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for AppletreeMSP   #####"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/AppletreeMSPanchors.tx -channelID $CHANNEL_NAME -asOrg AppletreeMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for AppletreeMSP..."
    exit 1
  fi
  echo
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for eHealthMSP   #######"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/eHealthMSPanchors.tx -channelID $CHANNEL_NAME -asOrg eHealthMSP
  res=$?
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for SunlifeMSP   #######"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/SunlifeMSPanchors.tx -channelID $CHANNEL_NAME -asOrg SunlifeMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for SunlifeMSP..."
    exit 1
  fi
  echo
}
